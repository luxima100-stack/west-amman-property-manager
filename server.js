...
app.get("/health",(req,res)=>res.json({
  ok:true,service:"west-amman-property-manager",version:"20.1.0",supabaseConfigured:configured()
}));
...
app.patch("/api/users/:id/permissions",owner,async(req,res)=>{
  try{
    const target=await sb(`/rest/v1/profiles?id=eq.${encodeURIComponent(req.params.id)}&select=id,role`);
    if(!target?.[0] || target[0].role!=="admin") return res.status(400).json({ok:false,message:"يمكن تعديل صلاحيات المديرين فقط."});
    const permissions=Array.isArray(req.body?.permissions)
      ? req.body.permissions.filter(x=>["dashboard","properties","tenants","reports","settings","messages"].includes(x))
      : [];
...
app.listen(PORT,"0.0.0.0",()=>console.log(`West Amman Property Manager 20.1.0 on ${PORT}`));
