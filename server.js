...
app.get("/health",(req,res)=>res.json({
  ok:true,service:"west-amman-property-manager",version:"19.1.0-owner-only",supabaseConfigured:configured()
}));
...
app.post("/api/login",async(req,res)=>{
  try{
    ...
    if(OWNER_EMAIL && email!==OWNER_EMAIL) return res.status(403).json({ok:false,message:"الدخول متاح للمالك فقط."});
    ...
    if(!p || p.role!=="owner")
      return res.status(403).json({ok:false,message:"الدخول متاح للمالك فقط."});
...
app.listen(PORT,"0.0.0.0",()=>console.log(`West Amman Property Manager 19.1.0 owner-only on ${PORT}`));
