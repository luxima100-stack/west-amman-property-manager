...
function searchForm(){
  ...
  return `<form id="searchForm" class="filters">
    ...
    <div class="field"><label>المنطقة</label><select id="area">${["الكل",...new Set([...(state.settings.areas||[]),...AREAS.slice(1)])].map(x=>`<option ${x===f.area?"selected":""}>${esc(x)}</option>`).join("")}</select></div>
...
function settingsPage(){
  ...
  const areas=[...new Set([...(s.areas||[]),...AREAS.slice(1)])];
  return `<section class="panel"><div class="panel-title"><h2>الإعدادات</h2><button class="icon-btn closePage">✕</button></div>
    ...
    <hr>
    <div class="panel-title"><div><h3>مناطق غرب عمان</h3><div class="muted">يمكنك إضافة أو حذف المناطق المخصصة، وسيتم حفظها في Supabase.</div></div></div>
    <div class="form-grid"><div class="field"><label>إضافة منطقة</label><input id="newAreaName" placeholder="مثال: أم السماق الجنوبي"></div><div class="field" style="align-self:end"><button type="button" class="btn" id="addArea">＋ إضافة المنطقة</button></div></div>
    <div class="area-chips">${areas.map(a=>`<span class="area-chip">${esc(a)}${AREAS.includes(a)?"":` <button type="button" class="icon-btn area-remove" data-remove-area="${esc(a)}">×</button>`}</span>`).join("")}</div>
...
async function usersPage(){
  let users=[];
  try{users=await api("/api/users")}catch(e){return `<section class="panel"><div class="panel-title"><h2>إدارة المديرين</h2><button class="icon-btn closePage">✕</button></div><p>${esc(e.message)}</p></section>`}
  return `<section class="panel"><div class="panel-title"><div><h2>إدارة المديرين والصلاحيات</h2><div class="muted">الصلاحيات باللغة العربية — المالك فقط يستطيع تعديلها أو تغيير كلمة سر المالك.</div></div><div class="btn-row"><button class="btn" id="newAdmin">＋ إضافة مدير</button><button class="icon-btn closePage">✕</button></div></div>
    <div class="security-owner-box"><h3>🔐 كلمة سر المالك</h3><p class="muted">تغيير كلمة مرور حساب المالك مباشرة من هذه الصفحة.</p><form id="ownerPasswordForm" class="form-grid"><div class="field"><label>كلمة السر الجديدة</label><input id="ownerPassword" type="password" minlength="6" required></div><div class="field" style="align-self:end"><button class="btn">تعديل كلمة سر المالك</button></div></form></div>
    <div class="table-wrap"><table class="table"><thead><tr><th>الاسم</th><th>البريد</th><th>الدور</th><th>الصلاحيات</th><th>الإجراء</th></tr></thead><tbody>
    ${users.map(u=>`<tr><td>${esc(u.name)}</td><td>${esc(u.email)}</td><td>${u.role==="owner"?"المالك":"مدير"}</td><td>${u.role==="owner"?"كامل":(u.permissions||[]).map(x=>PERMISSIONS.find(p=>p[0]===x)?.[1]||x).join("، ")||"لا توجد"}</td><td>${u.role==="admin"?`<button class="ghost-btn" data-edit-admin="${esc(u.id)}">تعديل الصلاحيات</button>`:"—"}</td></tr>`).join("")}
    </tbody></table></div></section>`;
}
function adminPermissionsModal(u){
  const current=new Set(u.permissions||[]);
  modal(`<div class="modal-head"><h2>صلاحيات المدير: ${esc(u.name)}</h2><button class="icon-btn closeModal">✕</button></div><form id="permForm" class="form-grid"><div class="field full"><label>حدد الصلاحيات</label><div class="checks">${PERMISSIONS.map(p=>`<label><input type="checkbox" value="${p[0]}" ${current.has(p[0])?"checked":""}> ${p[1]}</label>`).join("")}</div></div><div class="full btn-row"><button class="btn">حفظ الصلاحيات</button></div></form>`);
  $("#permForm").addEventListener("submit",async e=>{e.preventDefault();try{const permissions=$$("#permForm input[type=checkbox]:checked").map(x=>x.value);await api(`/api/users/${encodeURIComponent(u.id)}/permissions`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({permissions})});$("#modal-root")?.remove();toast("تم حفظ صلاحيات المدير");render()}catch(err){toast(err.message,false)}});
}
...
function details(p){
  ...
  $("#waShare")?.addEventListener("click",()=>shareWhatsApp(p,text));
}
async function shareWhatsApp(p,text){
  try{
    const urls=(p.images||[]).slice(0,10).filter(Boolean);
    if(navigator.share && urls.length){
      const files=[];
      for(let i=0;i<urls.length;i++){
        const r=await fetch(urls[i]); if(!r.ok) continue;
        const blob=await r.blob(); const ext=(blob.type.split('/')[1]||'jpg').replace('jpeg','jpg');
        files.push(new File([blob],`شقة-${p.code}-${i+1}.${ext}`,{type:blob.type||'image/jpeg'}));
      }
      if(files.length && (!navigator.canShare || navigator.canShare({files}))){await navigator.share({title:`شقة ${p.code}`,text,files});return}
      await navigator.share({title:`شقة ${p.code}`,text});return;
    }
    const n=String(state.settings.whatsapp||"").replace(/\D/g,"");
    if(n) location.href=`https://wa.me/${n}?text=${encodeURIComponent(text)}`;
    else toast("أضف رقم واتساب في الإعدادات أولاً",false);
  }catch(e){if(e?.name==='AbortError')return;toast("تعذر إرفاق الصور تلقائياً على هذا المتصفح؛ سيتم فتح واتساب بالنص",false);const n=String(state.settings.whatsapp||"").replace(/\D/g,"");if(n) location.href=`https://wa.me/${n}?text=${encodeURIComponent(text)}`;}
}
function propertyModal(p=null){
  ...
  modal(`<div class="modal-head"><h2>${p?"تعديل الشقة":"إضافة شقة"}</h2><button class="icon-btn closeModal">✕</button></div>
    <form id="propertyForm" class="form-grid">
      ...
      <div class="field"><label>المنطقة</label><select id="area">${[...new Set([...(state.settings.areas||[]),...AREAS.slice(1)])].map(x=>`<option ${x===p?.area?"selected":""}>${esc(x)}</option>`).join("")}</select></div>
...
function bindAdmin(){
  ...
  $("#addArea")?.addEventListener("click",async()=>{const name=$("#newAreaName").value.trim();if(!name)return toast("اكتب اسم المنطقة أولاً",false);if((state.settings.areas||[]).includes(name))return toast("المنطقة موجودة مسبقاً",false);state.settings.areas=[...(state.settings.areas||[]),name];await persistState();render();toast("تمت إضافة المنطقة وحفظها")});
  $$('[data-remove-area]').forEach(b=>b.addEventListener('click',async()=>{const name=b.dataset.removeArea;state.settings.areas=(state.settings.areas||[]).filter(x=>x!==name);await persistState();render();toast("تم حذف المنطقة") }));
  $$('[data-edit-admin]').forEach(b=>b.addEventListener('click',async()=>{try{const users=await api('/api/users');const u=users.find(x=>String(x.id)===String(b.dataset.editAdmin));if(u)adminPermissionsModal(u)}catch(e){toast(e.message,false)}}));
  $$("[data-view]").forEach(b=>b.addEventListener("click",()=>details(state.properties.find(p=>String(p.id)===String(b.dataset.view)))));
