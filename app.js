...
function loginView(){
  return `<div class="login"><div class="login-box"><div class="brand-large"><span class="brand-mark">⌂</span><b>عقارات غرب عمان</b></div><h1>تسجيل الدخول</h1><p>الدخول للمالك فقط</p><form id="loginForm"><div class="field"><label>البريد الإلكتروني</label><input id="email" type="email" required></div><div class="field"><label>كلمة المرور</label><input id="password" type="password" required></div><button class="btn">دخول إلى النظام</button><button type="button" class="ghost-btn" id="backHome">العودة</button></form></div></div>`;
}
function render(){
